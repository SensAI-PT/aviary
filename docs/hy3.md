# Hy3 (Tencent 295B MoE) on colibri

`c/hy3.c` runs [Tencent Hy3](https://huggingface.co/tencent/Hy3) (295B total / 21B active)
with colibri's expert-streaming approach: dense weights resident in RAM (~5 GB),
routed experts streamed from disk with an LRU + pinned cache. Auto-detected from
`config.json` (`model_type: hy_v3`); same `coli chat` / `coli serve` / `coli web` front end.
...
| Path | In git? | Purpose |
|------|---------|---------|
| `c/hy3_tiny/` (config, tokenizer stub, weights) | **no** | fully regenerated by `make_hy3_oracle.py` (gitignored as `c/hy3_tiny/`) |
| `c/hy3_tiny/tokenizer.json` | **generated** | stub byte-level vocab from `make_hy3_oracle.py` (needed by `coli`); smoke tests only — not a real Hy3 chat tokenizer |
...
`hy3_tiny` and `hy3_tiny_i4` are intentionally **not** committed (regenerable quickly).
...
python3 tools/make_hy3_oracle.py   # writes hy3_tiny/ (weights + tokenizer.json) and ref_hy3.json
make hy3
SNAP=./hy3_tiny TF=1 ./hy3 64 16 16    # expect 32/32 positions
```

`make_hy3_oracle.py` also writes a **stub** `tokenizer.json` (256 byte-level ids) so
`coli agent` / `coli serve` accept the fixture. That stub is for smoke tests only —
not a real Hy3 chat tokenizer.
