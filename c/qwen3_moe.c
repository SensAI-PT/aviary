/* Qwen3 MoE (qwen3_moe) streaming inference engine for Colibri.
 ...
 * MoE router: softmax + top-k + norm_topk_prob (Qwen3Moe style).
 *
 * Validation: TF=1 SNAP=./qwen3_moe_tiny ./qwen3_moe 64 16 16
 * Build: make qwen3_moe
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
typedef struct {
    ...
    int first_dense; /* 0 = all MoE layers; kept for shared telemetry.h */
    int norm_topk;
    float eps, theta;
...
typedef struct {
    ...
    QT q, k, v, o;
    float *qn, *kn;
    int sparse; /* always 1 for qwen3_moe; kept for shared telemetry.h */
    float *gate;
} Layer;
...
typedef struct {
    ...
    uint32_t **eheat;
    float *hlast, *h_all;
    int has_mtp; /* always 0; kept for shared telemetry.h */
...
static void load_cfg(Cfg *c, const char *snap){
    ...
    if(!mt||mt->t!=J_STR||!strstr(mt->str,"qwen3_moe")){
        fprintf(stderr,"config.json: model_type must be qwen3_moe\n"); exit(1);
    }
    ...
    c->n_experts=gi(r,"num_experts"); if(!c->n_experts) c->n_experts=gi(r,"num_local_experts"); c->topk=gi(r,"num_experts_per_tok");
    ...
    if(!th){ jval *rp=json_get(r,"rope_parameters"); th=rp?json_get(rp,"rope_theta"):NULL; }
    c->theta=th?(float)th->num:1000000.f;
    c->n_stop=0;
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    Cfg *c=&m->c; char nm[512]; int D=c->hidden, hd=c->head_dim;
    ...
    rt_init("qwen3_moe",c->n_layers,c->n_experts);   /* owns .coli_usage counters */
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    ...
    for(int s=0;s<S;s++){
        ...
        if(c->norm_topk){ float sm=1e-20f; for(int kk=0;kk<Ke;kk++) sm+=w[kk]; for(int kk=0;kk<Ke;kk++) w[kk]/=sm; }
        keff[s]=Ke; m->ereq+=Ke;
    ...
    free(logit); free(idxs); free(ws); free(keff); free(uniq);
...
static int spec_decode(Model *m, int *all, int kv, int n_new, int eos, float *logit,
    ...
    while(emitted<n_new&&!done){
        ...
        int g=0;
        if(gd>0){ g=ngram_draft(all,kv+1,gd,draft); }
...
static void cuda_fill_pinned(Model *m){
    ...
    double reserve=5.12e8; /* 512 MB headroom; 2 GB starved WSL occupancy */
    int skipped_cap=0;
    ...
    for(int li=0;li<c->n_layers && m->gpu_expert_bytes<budget;li++){
        ...
        for(int z=0;z<(m->npin?m->npin[li]:0) && m->gpu_expert_bytes<budget;z++){
            ESlot *s=&m->pin[li][z];
            int64_t before=m->gpu_expert_bytes;
            if(s->g.cuda_eligible && s->g.cuda) continue;
            int64_t need=qt_bytes(&s->g)+qt_bytes(&s->u)+qt_bytes(&s->d);
            if(before+need>budget) break;
        ...
        if(!m->ecache || !m->ecn) continue;
        for(int z=0;z<m->ecn[li] && m->gpu_expert_bytes<budget;z++){
...
static void run_text(Model *m, const char *snap, const char *prompt, int ngen){
    ...
    char wrapped[1<<16]; int bl=qwen3_wrap(wrapped,sizeof(wrapped),prompt,think);
    ...
    printf("speculation: %.2f tokens/forward (%llu forwards per %llu tokens)\n",
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(raw_mode){
            ...
            if(templ) bl=qwen3_wrap(buf,1<<16,input,think);
            ...
            if(len+k+req_ngen+2>=maxctx){ len=0; first=1; kv_alloc(m,4096);
                ...
                bl=0; if(templ) bl=qwen3_wrap(buf,1<<16,input,think);
                else bl=snprintf(buf,1<<16,"%s",input);
...
static int mux_submit(Model *m, Tok *T, ServeCtx *ctx, ServeReq *req, int nctx, int maxctx, int eos){
    ...
    if(prefix<sc->len){ sc->len=prefix; }
    int add=nt-sc->len;
...
#include <fcntl.h>
    ...
    if(getenv("COLI_METAL")&&atoi(getenv("COLI_METAL"))){
        fprintf(stderr,"Metal was requested, but this binary is CPU-only; rebuild with: make qwen3_moe METAL=1\n");
        return 2;
    }
#endif
#ifdef COLI_CUDA
    ...
    if((getenv("COLI_CUDA") && atoi(getenv("COLI_CUDA"))) ||
       ...
       (getenv("CUDA_EXPERT_GB") && atof(getenv("CUDA_EXPERT_GB"))>0)){
        fprintf(stderr,"CUDA was requested, but this binary is CPU-only; rebuild with: make qwen3_moe CUDA=1\n");
        return 2;
    }
#endif
    printf("== Qwen3 MoE C engine, cache=%d experts/layer | experts@%d-bit dense@%d-bit ==\n",cap,ebits,dbits);
    ...
    const char *refpath=getenv("REF")?getenv("REF"):"ref_qwen3_moe.json";
    ...
    if(getenv("TF")){
        ...
        { int maxid=0; for(int i=0;i<nfull;i++) if(full[i]>maxid) maxid=full[i];
          if(m.c.vocab>1000&&maxid<1000&&!getenv("REF_FORCE")){
            fprintf(stderr,"ERRORE: ref_qwen3_moe.json is the tiny oracle (max id %d, vocab %d).\n"
                           "  Self-test: SNAP=./qwen3_moe_tiny TF=1 ./qwen3_moe 64 16 16\n",maxid,m.c.vocab);
            return 1;
          } }
